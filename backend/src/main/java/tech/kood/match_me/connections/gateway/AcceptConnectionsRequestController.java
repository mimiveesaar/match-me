...
public class AcceptConnectionsRequestController {
    ...
    private final AcceptConnection.Handler acceptConnectionCommandHandler;

    public AcceptConnectionsRequestController(AcceptConnection.Handler acceptConnectionCommandHandler) {
    ...
    public ResponseEntity<Response> acceptConnectionRequest(AcceptConnectionDTO request) {
        ...
        var internalRequest = new AcceptConnection.Request(request.connectionIdDTO(), userId);
        var result = acceptConnectionCommandHandler.handle(internalRequest);

//        if (result instanceof AcceptConnection.Result.Success success) {
//            return ResponseEntity.ok(new Response.Success());
//        } else if (result instanceof AcceptConnection.Result.InvalidRequest invalidRequest) {
//            return ResponseEntity.badRequest().body(new Response.InvalidRequest(invalidRequest.error()));
//        } else if (result instanceof AcceptConnection.Result.NotFound notFound) {
//            return ResponseEntity.status(404).body(new Response.NotFound());
//        } else if (result instanceof AcceptConnection.Result.AlreadyAccepted alreadyAccepted) {
