...
public final class GetRefreshTokenHandler {
     ...
     *   <li>Attempts to find a valid refresh token entity using the provided token and the current time.</li>
     *   <li>If the token is invalid or not found, returns an {@code InvalidToken} result.</li>
     *   <li>If the token is valid, maps it to a domain object and returns a {@code Success} result.</li>
     ...
     * </ul>
     *
     * @param request the {@link GetRefreshTokenRequest} containing the token and tracing information
    ...
    public GetRefreshTokenResults handle(GetRefreshTokenRequest request) {
        ...
        try {
            ...
            if (tokenEntity.isEmpty()) {
                var result = new GetRefreshTokenResults.InvalidToken(request.token(), request.tracingId());
                events.publishEvent(new GetRefreshTokenEvent(request, result));
                return result;
            }
